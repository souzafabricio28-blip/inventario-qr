...
def vincular_codigo(codigo, ean_produto, origem="bip", sobrescrever=True):
    ...
    with get_db() as conn:
        ...
        try:
            if not sobrescrever:
                cur = _exec(conn,
                    "SELECT origem FROM produto_codigos WHERE codigo = %s",
                    (codigo,))
                existe = cur.fetchone()
                origens_rt = ("rt_planilha", "rt_oficial")
                if existe:
                    origem_atual = existe["origem"] if isinstance(existe, dict) else existe[0]
                    if origem in origens_rt and origem_atual in origens_rt:
                        _exec(conn,
                            "UPDATE produto_codigos SET ean_produto = %s, origem = %s WHERE codigo = %s",
    ...
    with get_db() as conn:
        for c in candidatos:
            cur = _exec(conn,
                ...
                "WHERE v.codigo = %s AND v.origem NOT IN ('rt_planilha', 'rt_oficial')",
                (c,))
            row = cur.fetchone()
            if row:
                return dict(row)
            ...
            cur = _exec(conn,
                """SELECT * FROM produtos
                   WHERE editado_manual = 1
                     AND (ean ILIKE %s OR produto ILIKE %s OR marca ILIKE %s)
                   ORDER BY produto""",
                (f"%{search}%", f"%{search}%", f"%{search}%"))
        else:
            cur = _exec(conn,
...
def listar_aliases():
    ...
    with get_db() as conn:
        cur = _exec(conn, "SELECT codigo, ean_produto, origem FROM produto_codigos")
        return [dict(r) for r in cur.fetchall()]
...
def loja_da_sessao(nome):
    ...
    with get_db() as conn:
        cur = _exec(conn,
            "SELECT loja FROM sessoes_inventario WHERE nome = %s LIMIT 1",
            (nome,))
        row = cur.fetchone()
    if not row:
        return ""
    return row["loja"] if isinstance(row, dict) else row[0]
...
def atualizar_produto(ean, ean_novo=None, editado_manual=None, **kwargs):
    ...
    if editado_manual is not None:
        campos.append("editado_manual = %s")
...
def get_contagens(sessao="", cruzar=False, loja=""):
    ...
    with get_db() as conn:
        if cruzar:
            if sessao:
                ...
                cur = _exec(conn,
                    f"SELECT {cols} FROM produtos p "
                    f"LEFT JOIN inventario_contagem i ON i.ean = p.ean AND (i.loja = %s OR i.id IS NULL) "
                    f"GROUP BY {group} "
                    f"ORDER BY CASE WHEN COALESCE(SUM(i.quantidade_contada), 0) = 0 THEN 1 ELSE 0 END, p.produto",
                    (loja,))
            else:
    ...
    if pares:
        with get_db() as conn:
            where = " OR ".join("(ean = %s AND loja = %s)" for _ in pares)
...
def criar_sessao(nome, loja="RTJ"):
    with get_db() as conn:
        cur = _exec(conn,
            "INSERT INTO sessoes_inventario (nome, loja) VALUES (%s, %s) RETURNING id",
            (nome, loja or "RTJ"))
        conn.commit()
...
def sessao_mais_ativa(loja=""):
    ...
    loja = (loja or "").strip()
    where_loja = "AND i.loja = %s" if loja else ""
...
def excluir_produto_completo(ean):
    with get_db() as conn:
        ...
        _exec(conn, "DELETE FROM estoque_produto WHERE ean=%s", (ean,))
        _exec(conn, "DELETE FROM produtos WHERE ean=%s", (ean,))
        conn.commit()
...
def mover_estoque(ean, loja="", quantidade=0):
    ...
    with get_db() as conn:
        row = _exec(conn,
            "SELECT quantidade_loja, quantidade_estoque FROM estoque_produto WHERE ean=%s AND loja=%s",
            (ean, loja)).fetchone()
        if row is None:
        ...
        _exec(conn,
            "UPDATE estoque_produto SET quantidade_loja=%s, quantidade_estoque=%s, updated_at=CURRENT_TIMESTAMP WHERE ean=%s AND loja=%s",
            (ql, qe, ean, loja))
        conn.commit()
    return True, msg
        ...
        if search:
            rows = _exec(conn, f"""
                ...
                WHERE ep.loja = %s
                  ...
                  AND (ep.ean ILIKE %s OR COALESCE(p.produto, '') ILIKE %s)
...
def listar_estoque_zerados(search="", loja=""):
    ...
    with get_db() as conn:
        ...
        if search:
            rows = _exec(conn, f"""
                SELECT {cols}
                FROM produtos p LEFT JOIN estoque_produto ep ON ep.ean = p.ean AND ep.loja = %s
                WHERE (COALESCE(ep.quantidade_loja, 0) + COALESCE(ep.quantidade_estoque, 0)) <= 0
                  AND (p.ean ILIKE %s OR p.produto ILIKE %s)
...
def registrar_saida(ean, produto, quantidade, lote, data_vencimento, motivo, observacao, loja=""):
    loja = _normalizar_loja(loja)
    with get_db() as conn:
        _exec(conn, """INSERT INTO saidas_estoque (ean, produto, quantidade, lote, data_vencimento, motivo, observacao, loja)
        ...
        if row is not None:
            ...
            if ql >= ded:
                _exec(conn,
                    "UPDATE estoque_produto SET quantidade_loja = quantidade_loja - %s, updated_at = CURRENT_TIMESTAMP WHERE ean=%s AND loja=%s",
                    (ded, ean, loja))
            else:
                _exec(conn,
                    "UPDATE estoque_produto SET quantidade_loja = 0, updated_at = CURRENT_TIMESTAMP WHERE ean=%s AND loja=%s",
                    (ean, loja))
                restante = ded - ql
                _exec(conn,
                    "UPDATE estoque_produto SET quantidade_estoque = GREATEST(0, quantidade_estoque - %s), updated_at = CURRENT_TIMESTAMP WHERE ean=%s AND loja=%s",
                    (restante, ean, loja))
        else:
            _exec(conn, """INSERT INTO estoque_produto (ean, loja, quantidade_loja, quantidade_estoque)
                VALUES (%s, %s, 0, 0) ON CONFLICT (ean, loja) DO NOTHING""", (ean, loja))
        conn.commit()
